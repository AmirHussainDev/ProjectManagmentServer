...
export class EmployeeService {
    ...
    async getUserEmployeeDetails(organizationId: number, subOrganizationId: number, userId: number) {
    ...
    async getCurrentEmployeeSubordinates(organizationId: number, subOrganizationId: number, userId: number) {
    ...
    async updateAttendance(userObj: any): Promise<Attendance> {
        ...
        // Find and return the updated user
        const updatedEmployee = await this.attendanceRepository.findOneBy({ id: userObj.id });
        if (!updatedEmployee) {
            // Handle case where user is not found after update
            throw new Error("Employee not found after update");
        }
    ...
    async getAllAttendance(organizationId: number, subOrganizationId: number, employeeId: number) {
    ...
    async getCurrentDateAttendance(employeeId: number): Promise<Attendance | undefined> {
    ...
    async getCurrentEmployeeAttendance(employeeId: number): Promise<{ lastPayment: EmployeePayments, attendances: Attendance[] }> {
        ...
        try {
            ...
            for (let i = 0; i < employees.length; i++) {
                ...
                const paymentObject = await this.attendanceRepository
                    ...
                    .where("attendance.employee.id = :employeeId", { employeeId: employeeId })
